...
Usage:
    python3 match_heureka.py --input heureka-cz-research-template.csv --index heureka_cz_raw.csv

--input   PricePilot's own manual_import "Download template" export
          (columns: product_id, sku, vendor, title, size_ml, our_price, ...)
--index   Raw listings gathered by scrape_heureka_cz.py
          (columns: vendor_searched, title, price_czk, url)
--out     Results CSV path (default: heureka-cz-results.csv)
--out-ambiguous
          Ambiguous-candidates CSV path (default: heureka-cz-ambiguous.csv,
...
def load_products(path: str) -> list[dict]:
    ...
    out = []
    for row in rows:
        # Confirmed live on the Skroutz.gr catalog export: a "Dolce & Gabbana"
        # vendor cell round-tripped through some export step as the literal
        # text "Dolce &amp; Gabbana" -- html.unescape() undoes that. Left
        # un-decoded, the extra "amp" survives normalize_text as a real
        # token, so canon_brand() never recognizes it as Dolce & Gabbana at
        # all and every one of that brand's products silently gets an empty
        # candidate pool. Applied here too for parity with match_skroutz.py.
...
def load_raw_listings(path: str) -> list[dict]:
    ...
    for row in rows:
        ...
        price_raw = row.get("price_czk") or row.get("price") or ""
        try:
...
def main():
    ...
    parser.add_argument("--index", required=True, help="Raw listings CSV from scrape_heureka_cz.py")
    parser.add_argument("--out", default="heureka-cz-results.csv")
    parser.add_argument("--out-ambiguous", default="heureka-cz-ambiguous.csv")
